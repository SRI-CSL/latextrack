...
We recommend to avoid loading LTC automatically when opening \Code{.tex} files.  Therefore, do not add a hook from \Code{latex-mode} to \Code{ltc-mode} (usually done with \Code{add-hook}).   The reason is that our mode requires the \Code{latex-mode} to be fully executed before LTC works. The hooks are not guaranteed to be executed in particular order, so it is best to manually invoke \Code{ltc-mode} after you have opened a \Code{.tex} file.

\begin{figure}[t]
\centering
\mygraphics{width=\textwidth,height=.5\textheight,keepaspectratio}{figures/emacs-port}
\caption{Using \Code{M-x customize-variable <RET> ltc-port <RET>} to set LTC port number in Emacs} \label{fig:emacs-port}
\end{figure}
