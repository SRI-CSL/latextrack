...
Now, we will check out from this repository in a new directory, say \Code{independence} using the user credentials for \Code{adams} with the password \Code{ltc}.  The password is shown in bold but will not appear on the screen.
\begin{CodeVerbatim}[commandchars=\\\{\}]
