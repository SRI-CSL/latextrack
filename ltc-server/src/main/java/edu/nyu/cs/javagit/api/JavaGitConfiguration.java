...
public final class JavaGitConfiguration {
    ...
    */
        ...
        */
        ...
        if (!(gitPrefix.equals(""))) {
            ...
            if (!(new File(gitCommand).exists())) {
                throw new JavaGitException("Invalid path to git specified.");
            }
        }
        ...
        GitVersionResponse response = ProcessUtilities.runCommand(null,
                commandLine,
                new GitVersionParser(new GitVersionResponse()));
        String version = response.getVersion();
        if (!(isValidVersionString(version)))
            throw new JavaGitException("Error retrieving git version.");
        return version;
    }
     ...
     * {@link #setGitPath(java.io.File) setGitPath} was previously called.)
     *
     ...
     *                          directory.
     */
    public static void setGitPath(File path) throws JavaGitException {
        if (path != null) {
            CheckUtilities.checkFileValidity(path);

            if (!(path.isDirectory()))
                throw new JavaGitException("Path is not a directory: "+path);
        }

        gitVersion = determineGitVersion(path); // check to see if git works
     ...
     * @throws java.io.IOException      Thrown if the provided path does not exist.
    ...
    */
    ...
    private static class GitVersionParser extends AbstractParser<GitVersionResponse> {

        private final static Pattern VERSION_PATTERN = Pattern.compile("^\\s*git version (.+)$");

        protected GitVersionParser(GitVersionResponse response) {
            super(response);
        }
    ...
    */
    ...
    private static class GitVersionResponse extends AbstractResponse {
